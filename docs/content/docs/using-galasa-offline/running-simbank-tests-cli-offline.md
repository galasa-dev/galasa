---
title: "Running the sample SimBank tests offline"
---

As described in the section [Exploring Galasa SimBank](../running-simbank-tests/index.md), SimBank is a simulated mainframe banking application provided with Galasa against which you can configure and run a set of provided sample tests in preparation for running your own tests against a real mainframe application. The Galasa zipped distribution provides the resources to do this offline also.
...
The SimBank tests are located in the `maven` directory of the `isolated.zip` downloadable file. Complete the following steps to run the SimBankIVT test that is provided with Galasa. The following example uses SimBank OBR version `1.0.1`.
...
1. Open a terminal and run the SimBankIVT test locally by using the following example command, remembering to update the `--localMaven` flag value to the location of the `maven` directory that is provided as part of the _isolated.zip_  file that you downloaded, where the `--localMaven` flag refers to the `maven` directory inside the _isolated.zip_:
=== "Linux or macOS"

    ```shell
    galasactl runs submit local --log - \
    --obr mvn:dev.galasa/dev.galasa.simbank.obr/1.0.1/obr \
...
Explore the SimBankIVT test and the other SimBank tests in the following pages. Follow the flow of logic in these classes and understand more about the Java that is used to create them, including how to use Galasa annotations and review documented test methods.
