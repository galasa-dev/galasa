...
The Text Scan Manager provides the following methods through the `ITextScanner` interface:

### scan

```java
ITextScanner scan(String text, Pattern searchPattern, Pattern failPattern, int count)
    throws FailTextFoundException, MissingTextException, IncorrectOccurrencesException, TextScanException
```

Searches text for regex patterns. Checks for failure pattern first, then searches for the search pattern.

**Parameters:**
- `text` - The text to be searched
- `searchPattern` - The regex pattern to search for
- `failPattern` - Failure regex to search for (can be null)
- `count` - Minimum number of occurrences required

**Returns:** This text scanner for fluent calls

---

```java
ITextScanner scan(String text, String searchLiteral, String failLiteral, int count)
    throws FailTextFoundException, MissingTextException, IncorrectOccurrencesException, TextScanException
```

Convenience method for scanning with literal text instead of patterns.

---

```java
ITextScanner scan(ITextScannable scannable, Pattern searchPattern, Pattern failPattern, int count)
    throws FailTextFoundException, MissingTextException, IncorrectOccurrencesException, TextScanException
```

Searches a scannable object for regex patterns.

---

```java
ITextScanner scan(InputStream inputStream, Pattern searchPattern, Pattern failPattern, int count)
    throws FailTextFoundException, MissingTextException, IncorrectOccurrencesException, TextScanException
```

Searches an input stream line by line to prevent heap overflow. Cannot use multiline patterns.

### scanForMatch

```java
String scanForMatch(String text, Pattern searchPattern, Pattern failPattern, int occurrence)
    throws MissingTextException, IncorrectOccurrencesException, TextScanException
```

Searches text and returns the matched text for a specific occurrence.
...
Similar overloads exist for `String` literals, `ITextScannable` objects, and `InputStream` sources.


## Exceptions

The Text Scan Manager throws the following exceptions:

- `FailTextFoundException` - Thrown when the failure pattern is found in the text
- `MissingTextException` - Thrown when the search pattern is not found at all
- `IncorrectOccurrencesException` - Thrown when the number of occurrences doesn't match the expected count
- `TextScanException` - General exception for other scanning problems
