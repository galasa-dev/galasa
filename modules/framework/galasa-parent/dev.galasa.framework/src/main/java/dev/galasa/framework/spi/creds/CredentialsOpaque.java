...
public class CredentialsOpaque extends AbstractCredentials implements ICredentialsOpaque {
     ...
     * @param base64Data the data (encrypted from etcd, or plain base64 from file)
     ...
     * {@inheritDoc}
     *
     * Stores the opaque value base64-encoded under the property key
    ...
    private byte[] decodeAndValidate(String value) throws CredentialsException {
        if (value == null || value.isBlank()) {
            throw new CredentialsException("Opaque credential data cannot be null or empty");
        }
        try {
            return base64(value);
        } catch (IllegalArgumentException e) {
            throw new CredentialsException("Failed to base64-decode opaque credential data", e);
        }
    }
}
